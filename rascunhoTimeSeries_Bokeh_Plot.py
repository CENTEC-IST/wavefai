...
def butter_highpass(interval, sampling_rate, cutoff, order=5):
    ...
    k=0;ax.plot_date(datn[ind],gaussian_filter(obs[ind],2.),'-',label=fnames[k],color=scolors[k],linewidth=2.)
    p.line(datn2[ind],gaussian_filter(obs[ind],2.), line_width=3, color=scolors2[k], alpha=0.8, legend_label=fnames[k])
    k=1;ax.plot_date(datn[ind],gaussian_filter(gfs[ind],2.),'-',label=fnames[k],color=scolors[k],linewidth=1.)
    p.line(datn2[ind],gaussian_filter(gfs[ind],2.), line_width=2, color=scolors2[k], alpha=0.8, legend_label=fnames[k])
    k=2;ax.plot_date(datn[ind],gaussian_filter(ecm[ind],2.),'-',label=fnames[k],color=scolors[k],linewidth=1.)
    p.line(datn2[ind],gaussian_filter(ecm[ind],2.), line_width=2, color=scolors2[k], alpha=0.8, legend_label=fnames[k])
    k=3;ax.plot_date(datn[ind],gaussian_filter(twc[ind],2.),'-.',label=fnames[k],color=scolors[k],linewidth=1.)
    p.line(datn2[ind],gaussian_filter(twc[ind],2.), line_width=2, color=scolors2[k], alpha=0.8, legend_label=fnames[k])
    k=4;ax.plot_date(datn[ind],gaussian_filter(mlv1[ind],2.),'--',label=fnames[k],color=scolors[k],linewidth=1.)
    p.line(datn2[ind],gaussian_filter(mlv1[ind],2.), line_width=2, color=scolors2[k], alpha=0.8, legend_label=fnames[k])
    k=5;ax.plot_date(datn[ind],gaussian_filter(mlv2[ind],2.),'--',label=fnames[k],color=scolors[k],linewidth=1.)
    p.line(datn2[ind],gaussian_filter(mlv2[ind],2.), line_width=2, color=scolors2[k], alpha=0.8, legend_label=fnames[k])
    k=6;ax.plot_date(datn[ind],gaussian_filter(mixp[ind],2.),':',label=fnames[k],color=scolors[k],linewidth=1.)
    p.line(datn2[ind],gaussian_filter(mixp[ind],2.), line_width=2, color=scolors2[k], alpha=0.8, legend_label=fnames[k])
    plt.legend(loc='upper left')
    plt.ylabel('VelVenMedSup', fontsize=sl)
    plt.xlabel("Tempo", fontsize=sl)
    for label in ax.get_xticklabels() + ax.get_yticklabels():
        label.set_fontsize(sl)
    plt.grid()
    plt.tight_layout();plt.axis('tight')
    plt.savefig('TimeSeriesObsvForecasts_'+stname+'_D'+str(d+1).zfill(2)+'.png', dpi=300, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format='png',transparent=False, bbox_inches='tight', pad_inches=0.1)
    plt.close(fig); del fig
    p.legend.location = "top_left" ; p.legend.click_policy = "hide"
    p.xaxis.axis_label = 'VelVenMedSup'; p.yaxis.axis_label = 'Tempo'
    output_file('TimeSeriesObsvForecasts_'+stname+'_D'+str(d+1).zfill(2)+'.html'); show(p); del p
    ...
    output_file('PDF_'+stname+'_D'+str(d+1).zfill(2)+'.html'); show(p); del p
    ...
    output_file('QQplot_'+stname+'_D'+str(d+1).zfill(2)+'.html'); show(p); del p
