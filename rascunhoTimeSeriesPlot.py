...
x=np.zeros(ffct.shape[0],'f')*np.nan
...
terrem=np.zeros((8,stations.shape[0],8,eftd.shape[0]),'f')*np.nan # array for error metrics, ensemble mean
terrms=np.zeros((8,stations.shape[0],8,ensm.shape[0],eftd.shape[0]),'f')*np.nan # array for error metrics, all members
terremd=np.zeros((8,stations.shape[0],8,eftd.shape[0]),'f')*np.nan # array for error metrics, ensemble mean from daily mean
# loop through forecast lead time
for d in range(0,eftd.shape[0]):
	...
	for i in range(0,stations.shape[0]):
		...
		a=np.array(np.nanmean(emsld[i,:,:,d],axis=0)); b=omsld[i,:,d]; inde=np.where(a*b>-999)
		terremd[:,i,0,d] = merr.metrics(a[inde],b[inde]); del inde,a,b
		a=np.array(np.nanmean(eatmpd[i,:,:,d],axis=0)); b=oatmpd[i,:,d]; inde=np.where(a*b>-999)
		terremd[:,i,1,d] = merr.metrics(a[inde],b[inde]); del inde,a,b
		a=np.array(np.nanmean(erhd[i,:,:,d],axis=0)); b=orhd[i,:,d]; inde=np.where(a*b>-999)
		terremd[:,i,2,d] = merr.metrics(a[inde],b[inde]); del inde,a,b
		a=np.array(np.nanmean(ewspad[i,:,:,d],axis=0)); b=owspad[i,:,d]; inde=np.where(a*b>-999)
		terremd[:,i,3,d] = merr.metrics(a[inde],b[inde]); del inde,a,b
		a=np.array(np.nanmean(ewdirad[i,:,:,d],axis=0)); b=owdirad[i,:,d]; inde=np.where(a*b>-999)
		terremd[:,i,4,d] = merr.metrics(a[inde],b[inde]); del inde,a,b
		a=np.array(np.nanmean(ewspbd[i,:,:,d],axis=0)); b=owspbd[i,:,d]; inde=np.where(a*b>-999)
		terremd[:,i,5,d] = merr.metrics(a[inde],b[inde]); del inde,a,b
		a=np.array(np.nanmean(ewspcd[i,:,:,d],axis=0)); b=owspcd[i,:,d]; inde=np.where(a*b>-999)
		terremd[:,i,6,d] = merr.metrics(a[inde],b[inde]); del inde,a,b
		a=np.array(np.nanmean(ewdircd[i,:,:,d],axis=0)); b=owdircd[i,:,d]; inde=np.where(a*b>-999)
...
for i in range(0,size(Nvar)):
	for l in range(0,8):
	
		# Maranhao
		fig1 = plt.figure(1,figsize=(7,6)); ax = fig1.add_subplot(111)
		for j in range(0,size(stations[indst][indstma])):
			ax.plot(eftd,gaussian_filter(terrem[l,indst[indstma][j],i,:], 1.), color=scolors[indstma[j]],label=fnames[indstma[j]], linewidth=2.,zorder=2)
			for k in range(0,ensm.shape[0]):
				ax.plot(eftd,gaussian_filter(terrms[l,indst[indstma][j],i,k,:], 1.), color=scolors[indstma[j]],linewidth=0.1,zorder=1)

		plt.legend()
		ax.set_xlabel('Forecast Time (Days)',size=sl); ax.set_ylabel(nerrm[l]+' '+Nvar[i],size=sl)
		plt.tight_layout();plt.axis('tight') 
		plt.grid(c='k', ls='-', alpha=0.3)
		plt.xticks(np.array([1,5,10,15,20,30,40])); plt.xlim(xmin = 0.9, xmax = eftd[-1]+0.1)
		...
		# Piaui
		fig1 = plt.figure(1,figsize=(7,6)); ax = fig1.add_subplot(111)
		for j in range(0,size(stations[indst][indstpi])):
			ax.plot(eftd,gaussian_filter(terrem[l,indst[indstpi][j],i,:], 1.), color=scolors[indstpi[j]],label=fnames[indstpi[j]], linewidth=2.,zorder=2)
			for k in range(0,ensm.shape[0]):
				ax.plot(eftd,gaussian_filter(terrms[l,indst[indstpi][j],i,k,:], 1.), color=scolors[indstpi[j]],linewidth=0.1,zorder=1)
	...
	np.savetxt(ifile,np.atleast_2d(eftd),fmt="%3i",delimiter=' ')
	ifile.write('# \n')
	for l in range(0,8):
